...
const RafAI = () => {
        ...
        messages.map((item, index) => (
          <section ref={messageRef} key={index} className={`${item.role === "user" ? "self-end max-w-[80%] bg-slate-700 rounded-md p-2 px-3" : "self-start"}`}>
            {item.role === "user" ? (
                ...
                <section ref={messageRef} className="w-full overflow-x-auto">
  ...
  return (
    <main className="h-screen overflow-hidden">
        ...
        <section className="container lg:w-11/12 mx-auto mt-3 mb-2 pb-4 text-white flex flex-col justify-between h-4/5 relative">
            ...
            <form action="" className="flex fixed left-0 bottom-0 right-0 mx-auto gap-2 p-2 items-center bg-slate-800 rounded-md" onSubmit={handleSubmit}>
                {file && (
